...
An interactive dashboard is built using **Streamlit** to demonstrate real-time news anomaly detection and location verification.

### ⚙️ Features

* 📰 Input news article (manual or dataset selection)
* 🌍 Predict source location using trained ML model
* 📊 Compute anomaly score (linguistic + statistical + temporal)
* 🧠 Validate source using NER-based location extraction
* 💬 Perform sentiment analysis (VADER)
* 🎯 Confidence-based decision system
* 📈 Visual breakdown of anomaly components
...
2. Text is converted into embeddings + keyword features
3. Model predicts location with confidence score
4. Anomaly score is calculated using:

   * Cosine similarity (linguistic deviation)
   * Isolation Forest (statistical anomaly)
   * Location statistics (temporal deviation)
5. NER extracts location entities for verification
